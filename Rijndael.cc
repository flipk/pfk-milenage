...
const uint8_t Rijndael :: Xtime[256] = {
...
Rijndael :: Rijndael( uint8_t key[16] )
{
    KeySchedule(key);
}

void Rijndael :: KeySchedule( uint8_t key[16] )
{
  uint8_t roundConst;
  int i, j;
...
void Rijndael :: KeyAdd(int round)
{
  int i, j;
...
void Rijndael :: Encrypt( uint8_t input[16], uint8_t output[16] )
{
  int i, r;
